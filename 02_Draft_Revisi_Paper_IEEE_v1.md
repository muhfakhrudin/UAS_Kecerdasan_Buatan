...
## Update — Aplikasi Berubah Sejak v3: 5 Kueri Resmi, Halaman Evaluasi Direstrukturisasi (v4)

Sejak revisi v3 di atas ditulis, tim mengubah aplikasi secara langsung (bukan revisi dokumen ini yang mendahului kode — kode berubah dulu, dokumen ini menyusul menyesuaikan). Tiga perubahan yang berdampak pada seluruh angka dan deskripsi fitur di paper:

1. **Query uji resmi dikurangi dari 14 menjadi 5 kueri.** Keputusan tim: 14 kueri dianggap terlalu banyak untuk dipertahankan manual. Set 5 kueri baru (`recommender/evaluation.py`, `QUERY_CONSTRAINTS`) sengaja dipilih tetap mencakup tiap jenis constraint yang didukung `_matches()`: varian+storage, +platform, +ambang battery, +wilayah, +sertifikasi — bukan subset acak dari 14 kueri lama. **Seluruh angka Table II di draft v1–v3 (Precision@10 0,6071 dst.) sudah usang** dan digantikan angka baru di bawah, dihasilkan ulang langsung dari `python manage.py run_evaluation` saat revisi v4 ini ditulis (lihat `eval/results.md`).
2. **Halaman `/evaluasi/paper/` ("Evaluasi Resmi") sudah dihapus dari aplikasi.** Alasan: halaman ini duplikatif dengan `python manage.py run_evaluation` (menampilkan angka yang sama), dan tim memutuskan angka resmi paper cukup bersumber dari command tersebut saja. **Setiap referensi ke halaman ini di draft v1–v3 sudah usang** — sudah diperbaiki di seluruh dokumen (Bab III.F, IV-A, IV-D) supaya tidak mendeskripsikan halaman yang tidak lagi ada.
3. **Halaman `/evaluasi/` ("Bandingkan BM25 vs TF-IDF") disederhanakan jadi demo satu kueri saja** (sebelumnya mendukung multi-kueri bebas via tombol "Tambah Query"). Kapabilitas multi-kueri dipindah ke halaman baru **Evaluasi Multi-Query** (`/evaluasi/multi-query/`), yang mensyaratkan tepat 5 kueri per eksperimen (bukan 1–10 bebas seperti sebelumnya). **Tabel III (sanity check) di draft v1–v3 memakai 3 kueri via `/evaluasi/` — ini tidak lagi bisa direproduksi persis** karena `/evaluasi/` sekarang hanya menerima 1 kueri. Tabel III sudah dijalankan ulang dengan 5 kueri bebas via `/evaluasi/multi-query/` (angka baru di Bab IV-E), bukan 3 kueri seperti sebelumnya.
4. **Efek pada narasi Hit Rate@10**: pada set 5 kueri resmi yang baru, TF-IDF mencapai Hit Rate@10 sempurna (1,0000) — berbeda dari set 14 kueri lama di mana TF-IDF hanya 0,9286. Klaim "Hit Rate@10 BM25 unggul dari TF-IDF" di draft v1–v3 (Bab IV-D, IV-F, Kesimpulan) **sudah tidak akurat untuk data baru** dan sudah ditulis ulang — keunggulan BM25 tetap jelas pada Precision/Recall/NDCG, hanya Hit Rate@10 yang kini imbang.

Prinsip yang sama dari v1 tetap dipegang: tidak ada angka yang dikarang. Seluruh angka baru di bawah adalah output nyata `python manage.py run_evaluation` dan halaman `/evaluasi/multi-query/`, dijalankan ulang saat revisi ini ditulis.
...
**[Diagram baru yang masih perlu dibuat: alur fitur perbandingan BM25 vs TF-IDF (`/evaluasi/`, demo satu kueri) dan Evaluasi Multi-Query (`/evaluasi/multi-query/`, 5 kueri sekaligus) — belum ada di paper manapun karena kedua fitur ini baru diimplementasikan setelah draft asli ditulis. Sarankan format serupa Fig. 1/2 (vertical flowchart berwarna) agar konsisten secara visual.]**
...
- **Query uji:** 5 kueri representatif yang mencakup variasi pencarian nyata pengguna (kombinasi seri/varian iPhone, kapasitas penyimpanan, platform, wilayah toko, dan tingkat kesehatan baterai), misalnya "iPhone 12 Pro 128GB Resmi" dan "iPhone 13 Pro battery diatas 90".
...
Hasil evaluasi aktual disajikan pada Bab IV-D dan IV-E, dihasilkan langsung oleh management command `python manage.py run_evaluation` (`recommender/evaluation.py`, dieksekusi ulang saat penyusunan bab ini — lihat `eval/results.md`) — tidak ada angka yang di-cache atau dikarang. Halaman web yang sebelumnya menampilkan angka ini secara live (`/evaluasi/paper/`) telah dihapus dari aplikasi karena duplikatif dengan command tersebut; angka Table II tetap 100% bersumber dari fungsi evaluasi yang sama persis, hanya jalur aksesnya yang berubah dari halaman web menjadi command-line.
...
Aplikasi memiliki tiga halaman utama yang telah diverifikasi berjalan nyata: (1) halaman pencarian utama dengan input kueri bebas, filter harga/kesehatan baterai/platform/tipe, dan mode tampilan terang-gelap (memenuhi KNF-2); (2) halaman **Bandingkan BM25 vs TF-IDF** (`/evaluasi/`) — demo satu kueri, menjalankan kedua metode secara live atas satu kueri bebas pilihan pengguna; dan (3) halaman **Evaluasi Multi-Query** (`/evaluasi/multi-query/`) yang menjalankan tepat 5 kueri bebas sekaligus dan menampilkan rerata Precision/Recall/Hit Rate/NDCG@10-nya sebagai eksperimen milik pengguna sendiri. Angka resmi Table II pada paper ini **tidak** bersumber dari halaman web manapun, melainkan dari management command `python manage.py run_evaluation` yang dijalankan langsung terhadap 5 kueri uji kurasi (Bagian III.F).
...
**Fig. 4.** Halaman Evaluasi Multi-Query (`/evaluasi/multi-query/`) menjalankan 5 kueri bebas sekaligus dan menampilkan rerata Precision/Recall/Hit Rate/NDCG@10 — dipakai untuk sanity check Tabel III, bukan sumber Table II *[screenshot perlu diambil ulang dari halaman ini setelah restrukturisasi v4]*
...
Evaluasi dijalankan terhadap 5 kueri uji berlabel (Bagian III.F) pada 526 produk asli, dengan K=10. Angka berikut adalah output langsung dari management command `python manage.py run_evaluation` (dieksekusi ulang saat penyusunan bab ini; rincian per kueri tersedia di `eval/results.md`) — tidak ada angka yang di-cache atau dikarang.

TABLE II. HASIL EVALUASI BM25 vs TF-IDF (RATA-RATA 5 KUERI, K=10)

| Method | Precision@10 | Recall@10 | Hit Rate@10 | NDCG@10 | Avg. Query Time (ms) |
|---|---|---|---|---|---|
| TF-IDF + Cosine Similarity | 0.3000 | 0.2559 | 1.0000 | 0.4039 | 25.653 |
| Okapi BM25 (Proposed) | 0.6600 | 0.4763 | 1.0000 | 0.8652 | 16.670 |

Okapi BM25 unggul dibandingkan TF-IDF pada Precision@10 (lebih dari dua kali lipat: 0,6600 vs 0,3000), Recall@10 (0,4763 vs 0,2559), dan NDCG@10 (lebih dari dua kali lipat: 0,8652 vs 0,4039), dengan waktu komputasi rata-rata juga lebih cepat (16,67 ms vs 25,65 ms per kueri). Hit Rate@10 kedua metode sama-sama sempurna (1,0000) pada 5 kueri ini — pada set kueri sebesar ini, TF-IDF selalu berhasil menemukan minimal satu produk relevan di Top-10, sehingga metrik ini tidak lagi membedakan kedua metode; keunggulan BM25 terlihat jelas pada Precision, Recall, dan NDCG yang mengukur seberapa BANYAK dan seberapa TINGGI PERINGKAT produk relevan ditemukan, bukan sekadar ada-tidaknya satu produk relevan.

**[Rincian hasil per kueri (5 baris) tersedia di `eval/results.md` / `eval/results.csv` (output `python manage.py run_evaluation`) dan dapat disisipkan sebagai tabel lampiran jika dosen meminta rincian granular saat presentasi]**
...
Sebagai pemeriksaan tambahan terhadap keandalan hasil pada Tabel II (bukan pengganti, melainkan pelengkap — Tabel II tetap acuan resmi), lima kueri bebas di luar 5 kueri kurasi resmi dijalankan melalui halaman **Evaluasi Multi-Query** aplikasi (`/evaluasi/multi-query/`, yang mensyaratkan tepat 5 kueri per eksperimen) untuk memastikan pola keunggulan BM25 tidak spesifik terhadap 5 kueri yang dikurasi. Kelima kueri bebas yang dipakai: "iPhone 13 256GB Tokopedia", "iPhone 12 Mini 128GB", "iPhone 11 Pro Max 256GB", "iPhone 13 Mini Shopee", dan "iphone second murah dibawah 3 juta".

TABLE III. SANITY CHECK — 5 KUERI DI LUAR SET RESMI (RATA-RATA 4 DARI 5 KUERI YANG TERDETEKSI ATRIBUT TERSTRUKTURNYA)

| Method | Precision@10 | Recall@10 | Hit Rate@10 | NDCG@10 |
|---|---|---|---|---|
| TF-IDF + Cosine Similarity | 0.4250 | 0.6562 | 1.0000 | 0.5535 |
| Okapi BM25 | 0.7250 | 1.0000 | 1.0000 | 0.9570 |

Pola yang sama (BM25 ≥ TF-IDF pada seluruh metrik yang terdefinisi) kembali terlihat. Kueri kelima dari sanity check ini ("iphone second murah dibawah 3 juta") tidak memiliki atribut terstruktur yang dapat dideteksi (tidak menyebut varian/storage/platform/battery/wilayah spesifik), sehingga metriknya tidak terdefinisi (bukan nol) dan dikecualikan dari rerata Tabel III — namun ditemukan observasi yang konsisten dengan temuan sebelumnya: TF-IDF + Cosine Similarity mengembalikan **0 hasil** untuk kueri ini, sementara BM25 tetap mengembalikan 10 hasil (dengan skor relevansi sangat rendah, 0,0014–0,0016). *[Penyebab pasti perbedaan ini — apakah TF-IDF memiliki ambang similarity minimum atau penanganan vektor kosong yang berbeda dari BM25 — perlu diverifikasi langsung di source code pada Tahap 3, bukan diasumsikan di sini.]*

## F. Diskusi

Hasil pada Tabel II dan III secara konsisten menunjukkan Okapi BM25 lebih unggul dibandingkan TF-IDF + Cosine Similarity pada Precision@10, Recall@10, dan NDCG@10, sekaligus dengan waktu komputasi yang lebih rendah — sedangkan Hit Rate@10 sama-sama sempurna (1,0000) untuk kedua metode pada seluruh kueri berlabel yang diuji. Pola keunggulan BM25 pada metrik-metrik tersebut konsisten baik pada 5 kueri kurasi resmi maupun pada 5 kueri bebas di luar set tersebut (Tabel III), sehingga tidak nampak sebagai artefak pemilihan kueri.

Kandidat penjelasan yang selaras dengan karakteristik BM25 (Bagian III.D.2) dan perlu diverifikasi lebih lanjut terhadap struktur dataset aktual: deskripsi produk pada dataset ini memiliki panjang yang bervariasi antar-penjual (disebutkan di Bab I sebagai sumber *keyword stuffing*), sehingga fungsi normalisasi panjang dokumen BM25 (parameter *b*) kemungkinan berperan signifikan dalam mencegah dokumen dengan deskripsi panjang namun repetitif memperoleh skor yang tidak proporsional — sesuatu yang tidak dimiliki TF-IDF murni. Meski Hit Rate@10 kedua metode sama-sama sempurna pada kueri berlabel yang terstruktur (setiap kueri berhasil menemukan minimal satu produk relevan di Top-10 baik oleh BM25 maupun TF-IDF), observasi pada kueri tanpa atribut terstruktur (Bagian IV-E) — TF-IDF mengembalikan nol hasil sama sekali, sementara BM25 tetap mengembalikan hasil dengan skor sangat rendah — mengindikasikan potensi selisih Hit Rate justru akan muncul pada kueri naturalistik yang lebih beragam di luar kueri berlabel yang terstruktur.

Kekuatan sistem: BM25 terbukti memberi hasil yang relevan (Precision@10 di atas 0,6) sekaligus responsif (rata-rata di bawah 17 ms per kueri), mendukung klaim sistem sebagai *decision support system* yang layak dipakai secara langsung oleh calon pembeli.
...
Hasil evaluasi terhadap 5 kueri uji berlabel pada 526 produk asli membuktikan Okapi BM25 unggul dibandingkan baseline TF-IDF + Cosine Similarity pada Precision@10 (0,6600 vs 0,3000), Recall@10 (0,4763 vs 0,2559), dan NDCG@10 (0,8652 vs 0,4039), dengan Hit Rate@10 sama-sama sempurna (1,0000) pada kedua metode — sekaligus waktu komputasi rata-rata BM25 lebih rendah (16,67 ms vs 25,65 ms per kueri). Keunggulan ini juga terkonfirmasi pada 5 kueri uji tambahan di luar set kurasi resmi (Tabel III), menunjukkan hasil tidak bergantung pada pemilihan kueri tertentu.
